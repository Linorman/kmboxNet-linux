...
static pthread_mutex_t m_hMutex_lock = PTHREAD_MUTEX_INITIALIZER; // 多线程互斥锁
...
int NetRxReturnHandle(client_tx* rx, client_tx* tx) // 接收的内容
{
    ...
    pthread_mutex_unlock(&m_hMutex_lock);
    return ret;             // 没有错误返回0
}
...
int kmNet_init(char* ip, char* port, char* mac)
{
    ...
    int err = sendto(sockClientfd, (const char*)&tx, sizeof(cmd_head_t), 0, (struct sockaddr*)&addrSrv, sizeof(addrSrv));
    usleep(20000); // 第一次连接可能时间比较久
    int clen = sizeof(addrSrv);
    err = recvfrom(sockClientfd, (char*)&rx, 1024, 0, (struct sockaddr*)&addrSrv, (socklen_t*)&clen);
...
int kmNet_mouse_move(short x, short y)
{
    ...
    pthread_mutex_lock(&m_hMutex_lock);
    ...
    err = recvfrom(sockClientfd, (char*)&rx, 1024, 0, (struct sockaddr*)&sclient, (socklen_t*)&clen);
    return NetRxReturnHandle(&rx, &tx);
}
...
void* ThreadListenProcess(void* lpParameter)
{
    int ret;
    sockMonitorfd = socket(AF_INET, SOCK_DGRAM, 0); // 创建套接字，SOCK_DGRAM指明使用 UDP 协议
    if (sockMonitorfd < 0)
        return (void*)-1;
    ...
    ret = bind(sockMonitorfd, (struct sockaddr*)&servAddr, sizeof(struct sockaddr));
    if (ret < 0)
        return (void*)-1;

    struct sockaddr cliAddr;  // 客户端地址信息
    int nSize = sizeof(struct sockaddr);
    char buff[1024];   // 缓冲区
    monitor_run = monitor_ok;
    while (1) 
    {
        int ret = recvfrom(sockMonitorfd, buff, 1024, 0, &cliAddr, (socklen_t*)&nSize); // 阻塞读数据
        if (ret > 0)
        {
...
int kmNet_monitor(short port)
{
    ...
    {   
        close(sockMonitorfd);
        sockMonitorfd = 0;
        usleep(100000); // 给点时间让线程先运行
    }
    if (port)
    {
        pthread_t thread_id;
        pthread_create(&thread_id, NULL, ThreadListenProcess, NULL);
    }
    return NetRxReturnHandle(&rx, &tx);
}
...
int kmNet_lcd_color(unsigned short rgb565)
{
    ...
    for (int y = 0; y < 40; y++)
    {
        ...
        struct sockaddr_in sclient;
        int clen = sizeof(sclient);
        err = recvfrom(sockClientfd, (char*)&rx, length, 0, (struct sockaddr*)&sclient, (socklen_t*)&clen);
    }
    return NetRxReturnHandle(&rx, &tx);
}
